...
default it denies when that artifact is missing or stale and tells you to run
`cc-bash-proxy verify`. You can opt in to the older convenience behavior with
`cc-bash-proxy hook --auto-verify`, but that lets hook-time config changes
regenerate the artifact without a separate review step.
