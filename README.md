...
The hook uses the verified artifact generated by `verify`. It fails closed when
the artifact is missing, stale, or incompatible, so policy edits, include edits,
Claude settings changes, and binary upgrades should be followed by
`cc-bash-guard verify`.
