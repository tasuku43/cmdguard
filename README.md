...
bundled into the artifact, but their contents are part of the artifact
fingerprint. The hook fails closed when the artifact is missing, stale, or
incompatible, so policy edits, include edits, Claude settings changes, and
binary upgrades should be followed by `cc-bash-guard verify`.
