...
artifact fingerprint. When the artifact is missing or stale, the hook
regenerates it only after the current effective config passes the same
verification checks as `cc-bash-guard verify`; if verification fails, the hook
returns `ask` with a warning instead of evaluating stale policy. Incompatible
artifacts still fail closed. Run `cc-bash-guard verify` directly when you want
to inspect diagnostics after policy edits, include edits, Claude permission
changes, or binary upgrades.
When a previous artifact can be compared, stale hook errors include the first
fingerprint input that appears to have changed, such as an included policy file,
Claude permission settings, or binary build metadata.
...
- regenerates missing or stale verified artifacts when verification passes,
  asks with a warning when verification fails, and fails closed when the
...
`cc-bash-guard verify`. When the artifact is missing or stale, the hook first
verifies the current effective config and regenerates the artifact only if
verification passes. If verification fails, the hook returns `ask` with a
warning `systemMessage` and `additionalContext`; it does not evaluate stale
policy. The hook still fails closed when the artifact is incompatible. Stale
hook warnings include a compact changed-input hint when the previous verified
