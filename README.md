...
- `cmdproxy test`, `cmdproxy check`, `cmdproxy doctor`, `cmdproxy verify`, `cmdproxy version`, and `cmdproxy hook claude`
...
`cmdproxy hook claude --rtk` evaluates `cmdproxy` rewrites first, checks Claude
permissions against the post-`cmdproxy` command, and only then applies the
final `rtk` rewrite. This avoids depending on Bash hook ordering.
